...
class ChartPainter extends BaseChartPainter {
  ...
  void drawCrossLineText(Canvas canvas, Size size) {
    ...
    TextPainter tp = getTextPainter(format(point.close), color: Colors.white54);
    ...
    TextPainter dateTp = getTextPainter(getDate(point.id!), color: Colors.white54);
...
//    canvas.drawCircle(Offset(x, y), 2.0, paintX);
  ...
  void drawRealTimePrice(Canvas canvas, Size size) {
    ...
    if (tp.width < max) {
      ...
      if (isLine) {
        ...
        canvas.drawCircle(
            Offset(x, y), 2.0, realTimePaint..color = Colors.white38);
      } else {
        stopAnimation(); //停止一闪闪
      }
  ...
  TextPainter getTextPainter(text, {color = Colors.grey}) {
